...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/registration?page=${page}`,
      {
        params:{
...
axios.delete(`/registration/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Matrícula</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos matrículas do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/registration/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        <td>#{{ index + 1 }}</td>
                                                        <td>{{ actualData.id}}/{{ actualData.schoolyear.year}}</td>
                                                        <td>{{ actualData.child_full_name}}</td>
                                                        <td>{{ actualData.sponsor_education_full_name}}</td>
                                                        <td>{{ actualData.sponsor_education_mobile}}</td>
                                                        <td>{{ actualData.schoolyear.year }}</td>
                                                        <td>
                                                            <span class="badge bg-warning" v-if="actualData.registration_status_id == 1">
                                                                Pendente
                                                            </span> 
                                                            <span class="badge bg-success" v-if="actualData.registration_status_id == 2">
                                                                Ativo
                                                            </span> 
                                                            <span class="badge bg-danger" v-if="actualData.registration_status_id == 3">
                                                                Inativo
                                                            </span> 
                                                        </td>
                                                       
                                                        <td>
                                                            <router-link :to="'/admin/registration/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/registration/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
